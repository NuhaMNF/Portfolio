...
import { SkillsConstellation } from "@/components/visualizations/SkillsConstellation";
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        {Object.entries(skills).map(([group, items]) => (
                          <div
                            key={group}
                            data-cursor="view"
                            className="rounded border border-zinc-800/60 bg-zinc-900/30 p-3"
                          >
                            ...
                            <div className="mt-1 flex flex-wrap gap-1.5">
                              {items.map((s) => (
                                <span
                                  key={s}
                                  className="rounded bg-zinc-900/80 px-1.5 py-0.5 font-mono text-[11px] text-zinc-300"
                                >
                                  {s}
                                </span>
                              ))}
